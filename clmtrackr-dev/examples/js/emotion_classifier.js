...
var emotionClassifier = function() {
	...
	this.predict = function(parameters) {
		var prediction = [];
		for (var j = 0;j < emotions.length;j++) {		// go through all emotions, use corresponding bias and coefficient to make predictions
			var e = emotions[j];
	...
	this.meanPredict = function (parameters) {
		...
		previousParameters.splice(0, previousParameters.length == 10 ? 1 : 0);		// sliding window
		previousParameters.push(parameters.slice(0));
		
		if (previousParameters.length > 9) {
			// calculate mean of parameters?
			var meanParameters = []		
			for (var i = 0;i < parameters.length;i++) {		// initialize meanParameters
				meanParameters[i] = 0;
			}
			for (var i = 0;i < previousParameters.length;i++) {		// get the sum of all 10 elements for each parameter
				for (var j = 0;j < parameters.length;j++) {
					meanParameters[j] += previousParameters[i][j];
				}
			}
			for (var i = 0;i < parameters.length;i++) {		// get the average for each parameter
				meanParameters[i] /= 10;
			}
