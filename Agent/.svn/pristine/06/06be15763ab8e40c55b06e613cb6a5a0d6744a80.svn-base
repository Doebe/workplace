...
//		female: false,			//OLD VERSIONS OF SWITCHING CHARACTERS
	...
	folder1.add(params, 'Happiness').min(0).max(100).step(1).listen().onChange(function(value){ 				mesh.morphTargetInfluences[ 48 ] = value/100;   		});
	folder1.add(params, 'Sadness').min(0).max(100).step(1).listen().onChange(function(value){ 				mesh.morphTargetInfluences[ 49 ] = value/100;   		});
	folder1.add(params, 'Surprise').min(0).max(100).step(1).listen().onChange(function(value){ 				mesh.morphTargetInfluences[ 50 ] = value/100;   		});
	folder1.add(params, 'Fear').min(0).max(100).step(1).listen().onChange(function(value){ 				mesh.morphTargetInfluences[ 51 ] = value/100;   		});
	folder1.add(params, 'Disgust').min(0).max(100).step(1).listen().onChange(function(value){ 				mesh.morphTargetInfluences[ 52 ] = value/100;   		});
	folder1.add(params, 'Anger').min(0).max(100).step(1).listen().onChange(function(value){ 				mesh.morphTargetInfluences[ 53 ] = value/100;   		});
...
function resetMorphs()
{
	...
	for(var i = 17 ; i < 48 ; i++) mesh.morphTargetInfluences[i] = 0;
}
