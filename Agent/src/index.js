! function e(t, n, r) {
  ...
  5: [function(require) {
    ...
    window.BUFFERSIZE = 8192, $(document).ready(function() {
      ...
      // showError("tokenGenerator: " + tokenGenerator "\n");
      tokenGenerator.getToken(function(err, token) {
        window.onbeforeunload = function() {
          localStorage.clear()
        }, token || (console.error("No authorization token available"), console.error("Attempting to reconnect..."), showError(err && err.code ? "Server error " + err.code + ": " + err.error : "Server error " + err.code + ": wtf4"));
        var viewContext = {
    ...
    var utils = require("./utils"),
      ...
      initSocket = exports.initSocket = function(options, onopen, onlistening, onmessage, onerror, onclose) {
        ...
        socket.onopen = function() {
          ...
          console.log("WS onerror: ", evt), showError("Application error " + evt.code + ": wtf1"), $.publish("clearscreen"), onerror(evt)
          ...
          return 1011 === evt.code ? (console.error("Server error " + evt.code + ": wtf2"), !1) : evt.code > 1e3 ? (console.error("Server error " + evt.code + ": wtf3"), !1) : ($.unsubscribe("hardsocketstop"), $.unsubscribe("socketstop"), void onclose(evt))
        }
      }
  }, {
    "./utils": 8,
  ...
  10: [function(require, module, exports) {
    ...
    exports.initDisplayMetadata = function() {
      ...
      if (msg.results && msg.results.length > 0) {
        ...
        msg.results && msg.results[0] && msg.results[0]["final"] ? (text = text.slice(0, -1), text = text.charAt(0).toUpperCase() + text.substring(1), japanese ? (text = text.trim() + "ã€‚", text = text.replace(/ /g, "")) : text = text.trim() + ". ", baseString += text, $("#resultsText").val(baseString), showMetaData(alternatives[0]), alternativePrototype.showAlternatives(alternatives)) : (text = japanese ? text.replace(/ /g, "") : text.charAt(0).toUpperCase() + text.substring(1), $("#resultsText").val(baseString + text))
      }
